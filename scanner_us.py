...
USO: python scanner_us.py    |    python scanner_us.py --quick
"""
...
def main():
    ...
    n=len(hits)
    html=f"""<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Scanner EUA</title>
    ...
    # JSON p/ TradeDesk USA: alvo = entrada + 2R (2x risco)
    trades=[]
    for h in hits:
        risco=h["close"]-h["stop"]
        alvo=round(h["close"]+2*risco,2) if risco>0 else ""
        trades.append({
            "data": today, "s": h["ticker"], "setup": SETUP,
            "q": "", "pc": str(h["close"]),
            "pa": "", "pv": "", "dv": "",
            "ps": str(h["stop"]),
            "tgt": str(alvo),
